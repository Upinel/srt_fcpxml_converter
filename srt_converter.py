...
PYTHON3 VERSION AUTHOR: NOVA ALTESSE
PYHTON2 VERSION AUTHOR: MICHAEL HONG
"""
...
parser.add_argument('-o', '--output', required=True, help="name for the output file (.srt or .fcpxml)")
...
def convert_t_xml(t):
    ...
    x = int(int(t * denominator) / multiplier) * multiplier
    if x % denominator == 0:
        return f'{x // denominator}s' ## whole number
...
def process_input_srt():
    ...
    while i < total_rows:
        i += 1
        m = re.match(r'(\d+):(\d+):(\d+),(\d+) --> (\d+):(\d+):(\d+),(\d+)', lines[i])
...
def process_output_fcpxml(data):
    ...
    if abs(args.framerate - xml_framerate_fps) > 0.005:
        raise Exception(f'template framerate {xml_framerate_fps}fps is inconsistent with specified framerate {args.framerate}fps. \
